...
async function readCSVFile() {
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream('medicines.csv')
      ...
      .on('data', (data) => medicines.push(data))
      .on('end', () => {
        console.log(`CSV file read successfully. Total records: ${medicines.length}`);
        resolve(medicines);
      })
      .on('error', (error) => {
...
async function importData() {
  try {
    const medicines = await readCSVFile();
    console.log(`Importing ${medicines.length} medicines into Elasticsearch...`);
    ...
    for (let i = 0; i < medicines.length; i += batchSize) {
      ...
      await client.bulk({ refresh: true, body });
      console.log(`Imported batch of medicines ${i + 1} to ${Math.min(i + batchSize, medicines.length)}`);
    }
...
async function initializeDatabase() {
  ...
  while (retries > 0) {
    try {
      ...
      retries--;
      console.error(`Database initialization failed. Retries left: ${retries}`, error);
      if (retries > 0) {
        console.log('Retrying in 5 seconds...');
        await new Promise(resolve => setTimeout(resolve, 5000));
      }
